...
//
// Separate Stripe webhook used only for transactional customer emails.
...
function verifyStripeSignature(body, signatureHeader) {
  ...
  if (!secrets.length) {
    const error = new Error(
      'Missing Stripe email webhook signing secret.'
    );
  ...
  if (
    ...
    const error = new Error(
      'Stripe webhook signature timestamp is outside the allowed window.'
    );
...
async function waitForPaidPayment(
  sessionId
) {
  for (
    let attempt = 1;
    attempt <= PAYMENT_WAIT_ATTEMPTS;
    attempt += 1
  ) {
    const rows =
      await supabase(
...
function bookedItemsRows(data) {
  ...
  if (data.packageInfo) {
    rows.push(`
      <tr>
        <td style="padding:9px 0;border-bottom:1px solid #e9e4d8;font-weight:700;">
          ${escapeHtml(data.packageInfo.name)} package
        </td>
        <td style="padding:9px 0;border-bottom:1px solid #e9e4d8;text-align:right;font-weight:700;">
          ${money(data.packageInfo.price)}
        </td>
      </tr>
    `);
  }
  ...
  data.items.forEach(item => {
    ...
    rows.push(`
      <tr>
        <td style="padding:9px 0;border-bottom:1px solid #e9e4d8;">
          ${escapeHtml(item.product_name_snapshot)}
        </td>
        <td style="padding:9px 0;border-bottom:1px solid #e9e4d8;text-align:right;">
          ${amount}
        </td>
      </tr>
    `);
...
<div style="font-size:16px;line-height:1.65;margin-bottom:26px;">
  ${intro}
</div>
...
<td style="padding:5px 0;text-align:right;font-weight:700;">
  ${escapeHtml(formatDate(booking.event_date))}
</td>
</tr>

<tr>
<td style="padding:5px 0;color:#667269;vertical-align:top;">
  Venue
</td>
<td style="padding:5px 0;text-align:right;font-weight:700;">
  ${escapeHtml(venueText(venue))}
</td>
</tr>
...
<td style="padding:5px 0;text-align:right;font-weight:700;">
  ${escapeHtml(setupLabel(booking.setup_preference))}
</td>
</tr>
...
<p style="margin:28px 0 0;font-size:14px;line-height:1.65;color:#667269;">
  Keep this email for your records. If you need to contact us, simply reply to this email.
</p>
...
<div style="font-size:12px;color:#788078;margin-top:10px;">
  You can pay at any time before the due date.
</div>
...
`
    ...
    return {
      ...
      subject:
        `Your wedding booking is confirmed — ${booking.booking_reference}`,
      ...
      html:
        emailShell({
          ...
          intro:
            `We've received your deposit of <strong>${money(payment.amount)}</strong> and your wedding date is now secured with On The Green Games.`,
...
`;
    ...
    return {
      emailType:
        'booking_confirmation_full',

      dedupeKey:
        `booking-confirmation/${booking.id}`,

      subject:
        `Your wedding booking is confirmed and paid in full — ${booking.booking_reference}`,

      html:
        emailShell({
          title:
            "You're booked and paid in full",

          eyebrow:
            'Booking confirmed',

          intro:
            `We've received your full payment of <strong>${money(payment.amount)}</strong>. Your wedding booking is confirmed and there is nothing else to pay.`,
...
`;
    ...
    return {
      ...
      dedupeKey:
        `balance-payment-confirmation/${payment.id}`,

      subject:
        `Your balance is paid — ${booking.booking_reference}`,
      ...
      html:
        emailShell({
          ...
          intro:
            `We've received your remaining balance payment of <strong>${money(payment.amount)}</strong>. Your booking is now paid in full.`,
...
async function claimDelivery({
  ...
  if (!existing) {
    throw new Error(
      'Could not load existing email delivery after a dedupe conflict.'
    );
  }
...
async function handler(event) {
  ...
  try {
    ...
    if (
      ...
      return response(
        200,
        {
          ...
          reason:
            'Not an OTGG payment session.'
        }
      );
    }

    // Stripe does not guarantee delivery order
    // across separate webhook endpoints.
    // Wait briefly for the existing payment
    // webhook to finish first.
    const payment =
      await waitForPaidPayment(
        session.id
      );
    ...
    if (
      ...
      throw new Error(
        'Stripe metadata does not match the OTGG payment row.'
      );
    }
